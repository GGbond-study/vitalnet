...
@MODELS.register_module()
class VTClsDataPreprocessor(BaseDataPreprocessor):
    """Image pre-processor for classification tasks.
    ...
    def __init__(self,
                 ...
                 num_classes: Optional[int] = None,
                 batch_augments: Optional[dict] = None,
                 table_mean: Sequence[Number] = None,
                 table_std: Sequence[Number] = None,):
        super().__init__()
        self.pad_size_divisor = pad_size_divisor
        self.pad_value = pad_value
        self.to_rgb = to_rgb
        ...
        if table_mean is not None and table_std is not None:
            self.table_mean = table_mean
            self.table_std = table_std
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        inputs = self.cast_data(data['inputs'])
        inputs_table = self.cast_data(data['inputs_table'])
        table = self.Normalization(inputs_table)
        ...
        return {'inputs': inputs, 'data_samples': data_samples, 'inputs_table':table}
    
    def Normalization(self, inputs_table):
        mean = torch.tensor(self.table_mean).to(self.device)
        std = torch.tensor(self.table_std).to(self.device)
        x = inputs_table - 0
        x = x / 1
        return x

@MODELS.register_module()
class VTTClsDataPreprocessor(BaseDataPreprocessor):
    """Image pre-processor for classification tasks.
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        inputs = self.cast_data(data['inputs'])
        inputs_text = self.cast_data(data['inputs_text'])
        inputs_table = self.cast_data(data['inputs_table'])
        #table = self.Normalization(inputs_table)
        ...
        return {'inputs': inputs, 'data_samples': data_samples, 'inputs_table':inputs_table, 'inputs_text':inputs_text}
