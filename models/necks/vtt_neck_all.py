...
class VTTAPooling(nn.Module):
    ...
    def __init__(self, dim=2, hidden_dim = 2048, Table = False, bert_model_name = None, num_classes = 6,
        ...
        super(VTTAPooling, self).__init__()
        ...
        if self.Table:
            ...
            saved_state_dict = self.load_model(tab_path)
    ...
    def forward(self, inputs, inputs_table, inputs_text):    
        ...
        if self.Table :
            ...
            outs_text = self.cross_attention_text(outs_, text_info) 
            outs = outs[-1] + 0.9*outs_text #+ outs_
            #print(text_info)
            cls_text = self.fc(text_info)  
            ################################################
            #outs = outs[-1]
            feat = {'feat': outs, 'feat_table': outs_table ,'feat_text': cls_text, 'feat_te': outs_text}
            outs = feat
