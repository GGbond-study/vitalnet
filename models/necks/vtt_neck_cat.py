...
class VTTCPooling(nn.Module):
    ...
    def __init__(self, dim=2, hidden_dim = 2048, Table = False, bert_model_name = None, num_classes = 8):
        super(VTTCPooling, self).__init__()
        ...
        if self.Table:
            ...
            saved_state_dict = self.load_model('/home/user/ZY_Workspace/Django_VTmamba/mmpretrain/tabnet_model(45.zip')
    ...
    def forward(self, inputs, inputs_table, inputs_text):    
        ...
        if self.Table :
            ...
            #print(type(outs[-1]), type(outs_table_feature), type(text_info))
            all = torch.cat((outs[-1], outs_table_feature), dim=1)
            out = [all]
            #print(text_info)
