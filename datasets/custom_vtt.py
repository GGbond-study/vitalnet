...
class VTTDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self.tokenizer = BertTokenizer.from_pretrained('/home/user/ZY_Workspace/Django_VTmamba/VTmamba/Bert-Chinese-Text-Classification-Pytorch-master.zip/bert-base-chinese-vocab.txt')
    ...
    def load_data_list(self):
        ...
        #df = pd.read_excel('/home/user/ZY_Workspace/Django_VTmamba/VTmamba/最终版超声数据（598例-表格+单侧超声）/（7类+其他）598例-有超声.xlsx', sheet_name='Sheet1')
        csv_file = './data/information_text.csv'
        selected_columns = ['age','bmi','hurt','zhang','CA125','CEA','CA199','AFP','CA153','size'] 
        vtt_columns = ['diagnose', 'report']
        # Pre-build file backend to prevent verbose file backend inference.
        ...
        for sample in samples:
            if self.with_label:
                ...
                table = self.get_patient_info(csv_file_path=csv_file, patient_id=filename, columns=selected_columns)
                table = torch.tensor(table)
                text = self.get_patient_vttinfo(csv_file_path=csv_file, patient_id=filename, columns=vtt_columns)
                #print(text)
                text_masked = self.tokenizer(text[0], padding='max_length', truncation=True, max_length=256, return_tensors='pt', is_split_into_words=False)
                #print(text_masked.keys())
                info = {'img_path': img_path, 'table': table, 'gt_label': int(gt_label), 'text': text_masked}
            else:
    ...
    def get_table(self, filename: str,
        ...
        idx = idx[:10]
        if int(idx) in all['患者编号'].values:
            idx = all[all['患者编号'] == int(idx)].index[0]
        table_data = []
        selected_data = [5, 15, 17, 18, 26, 27, 28, 29, 30, 34] #[age，bmi, 腹痛(0,1), 腹胀(0,1), 5个指标, 大径线]
        #selected_data = [17, 18, 26, 27, 28, 29, 30]
        for row in selected_data:
            selected_data = str(all.iloc[idx, row])
            if selected_data == '无' or selected_data == '否':
                selected_data = 0
                table_data.append(selected_data)
                continue
            elif selected_data == '有' or selected_data == '是':
                selected_data = 1
                table_data.append(selected_data)
                continue
            elif selected_data == '/':
                ...
                selected_data = self.extract_decimals_regex(selected_data)
                if selected_data == '':
                    selected_data = -1
                table_data.append(selected_data)
        float_list = list(map(float, table_data))
        table_data = torch.tensor(float_list)
        table_data[0] = table_data[0] / 100
        return table_data
    
    def get_patient_info(self, csv_file_path, patient_id, columns):
        idx = patient_id.split('/')[-1]#.split('-')[0]
        patient_id = int(idx[:10])
        try:
            with open(csv_file_path, 'r', newline='') as file:
                reader = csv.DictReader(file)
                for row in reader:
                    if row['idx'] == str(patient_id):
                        return [float(row[col]) for col in columns if col in row]
                print(f"未找到编号为 {patient_id} 的患者信息。")
                return []
    ...
    def get_patient_vttinfo(self, csv_file_path, patient_id, columns):
        idx = patient_id.split('/')[-1]#.split('-')[0]
        patient_id = int(idx[:10])
        try:
            with open(csv_file_path, 'r', newline='') as file:
                ...
                for row in reader:
                    if row['idx'] == str(patient_id):
                        return [str(row[col]) for col in columns if col in row]
                print(f"未找到编号为 {patient_id} 的患者信息。")
                return []
        except FileNotFoundError:
            print(f"错误: 文件 {csv_file_path} 未找到。")
    ...
    def extract_decimals_regex(self, text):
    # 使用正则表达式匹配所有包含小数点的数字
        pattern = r'[-+]?\d*\.\d+|[-+]?\d+'
    ...
    def get_patient_vttinfo_(self, csv_file_path, patient_id, columns):
        idx = patient_id.split('/')[-1]
        patient_id = int(idx[:10])
        try:
            with open(csv_file_path, 'r', newline='') as file:
                reader = csv.DictReader(file)
